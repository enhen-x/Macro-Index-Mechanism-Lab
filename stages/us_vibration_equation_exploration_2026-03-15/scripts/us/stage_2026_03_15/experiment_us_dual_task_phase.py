"""Step 20 experiment: dual-task post-model (amplitude regression + phase classification).

Task A (amplitude):
    residual_t = y_true_t - y_hat_raw_t
    residual_hat_t = ridge(X_t)
    y_hat_amp_t = y_hat_raw_t + residual_hat_t

Task B (phase):
    turn_prob_t = sigmoid(w_turn^T X_t)
    up_prob_t = sigmoid(w_up^T X_t)

Fusion:
    delta_amp_t = y_hat_amp_t - y_anchor_t
    g_t = clip((turn_prob_t - tau) / (1 - tau), 0, 1)
    delta_target_t = |delta_amp_t| * sign(up_prob_t - 0.5)
    delta_final_t = (1 - lambda*g_t) * delta_amp_t + (lambda*g_t) * delta_target_t
    y_hat_final_t = y_anchor_t + delta_final_t
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dual-task amplitude+phase post-model scan.")
    ...
    parser.add_argument("--output-dir", default="data/us/experiments/step20_dual_task_phase")
...
def _one_step_raw(rows: list[dict[str, str]], feature_cols: list[str], est: dict, dt: float) -> dict[str, np.ndarray]:
    ...
    # turn label: sign(delta_t) -> sign(delta_next) change
    turn_label = (
        (np.abs(delta_t) > 1e-6)
...
def _fit_logistic(
    ...
    *,
    ...
    if np.all(yb == 0.0) or np.all(yb == 1.0):
        return {
            "is_constant": True,
            "constant_prob": float(np.mean(yb)),
        }
...
def _build_final_prediction(
    ...
    *,
    ...
    p_up = _predict_logistic(ds["x_meta"], up_model)
    denom = max(1.0 - float(tau), 1e-8)
    g = np.clip((p_turn - float(tau)) / denom, 0.0, 1.0)
    sign_target = np.where(p_up >= 0.5, 1.0, -1.0)
...
def main() -> None:
    ...
    amp_target = fit["y_true"] - fit["y_hat_raw"]
    amp_model = _fit_ridge(fit["x_meta"], amp_target, alpha=float(args.ridge_alpha))
    turn_model = _fit_logistic(
        fit["x_meta"],
        fit["turn_label"],
        l2=float(args.logit_l2),
        lr=float(args.logit_lr),
        iters=int(args.logit_iters),
    )
    up_model = _fit_logistic(
        fit["x_meta"],
        fit["up_label"],
        l2=float(args.logit_l2),
        lr=float(args.logit_lr),
    ...
    for lam in lambda_grid:
        for tau in tau_grid:
            sel_pred = _build_final_prediction(sel, amp_model, turn_model, up_model, lam=float(lam), tau=float(tau))
            ...
            eva_pred = _build_final_prediction(eva, amp_model, turn_model, up_model, lam=float(lam), tau=float(tau))
    ...
    eva_pred = _build_final_prediction(
        eva,
        amp_model,
        turn_model,
        up_model,
    ...
    summary = {
        ...
        "fit_models": {
            ...
            "turn_fit_logloss": float(turn_model.get("fit_logloss", float("nan"))),
            "up_fit_logloss": float(up_model.get("fit_logloss", float("nan"))),
    ...
    (out_dir / "dual_task_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

    with (out_dir / "dual_task_scan.csv").open("w", encoding="utf-8", newline="") as f:
            ...
            writer.writerow(
                {
                    ...
                    "y_t1_pred_dual": float(eva_pred["y_final"][i]),
                    "p_turn": float(eva_pred["p_turn"][i]),
                    "p_up": float(eva_pred["p_up"][i]),
    ...
    print("[done] step20 dual-task phase scan complete")
    print(
        "[done] best_by_selection: "
        f"lambda={best_by_selection['lambda']:.6g}, tau={best_by_selection['tau']:.6g}, "
    ...
    print(
        "[done] best_by_eval: "
        f"lambda={best_by_eval['lambda']:.6g}, tau={best_by_eval['tau']:.6g}, "
