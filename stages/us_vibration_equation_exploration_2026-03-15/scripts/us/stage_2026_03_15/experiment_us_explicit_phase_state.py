"""Step 15 experiment: explicit phase-state model on top of one-step forecasts.

State-space style correction:
    z_t = rho * z_{t-1} + kappa * (y_hat_raw_t - y_t) + eta * (y_t - y_{t-1})
    y_hat_corr_t = y_hat_raw_t + z_t

The delay/phase effect is modeled by an explicit latent state z_t instead of
only static coefficients.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scan explicit phase-state parameters on top of one-step forecasts.")
    ...
    parser.add_argument("--output-dir", default="data/us/experiments/step15_explicit_phase_state")
    ...
    parser.add_argument("--rho-grid", default="0.0:0.95:0.05", help="start:end:step")
    parser.add_argument("--kappa-grid", default="-0.8:0.8:0.1", help="start:end:step")
    parser.add_argument("--eta-grid", default="-0.6:0.6:0.1", help="start:end:step")
...
def _apply_phase_state(
    ...
    *,
    ...
    y_corr = np.zeros(len(y_hat_raw), dtype=float)
    z_series = np.zeros(len(y_hat_raw), dtype=float)
    z_prev = 0.0
    do_clip = z_clip > 0
    for i in range(len(y_hat_raw)):
        delta_pred = float(y_hat_raw[i] - y_anchor[i])
        delta_anchor = float(y_anchor[i] - y_prev[i])
        z_t = rho * z_prev + kappa * delta_pred + eta * delta_anchor
        if do_clip:
            z_t = float(np.clip(z_t, -z_clip, z_clip))
        y_corr[i] = y_hat_raw[i] + z_t
        z_series[i] = z_t
        z_prev = z_t
    return y_corr, z_series
...
def main() -> None:
    ...
    eta_grid = _parse_grid(args.eta_grid)

    rows_scan: list[dict[str, float]] = []
    for rho in rho_grid:
        for kappa in kappa_grid:
            for eta in eta_grid:
                y_corr_sel, _ = _apply_phase_state(
                    sel["y_hat_raw"],
                    sel["y_anchor"],
                    sel["y_prev"],
                    rho=float(rho),
    ...
    kappa_best = float(best_row["kappa"])
    eta_best = float(best_row["eta"])

    y_raw = eva["y_hat_raw"]
    y_corr, z_corr = _apply_phase_state(
        y_raw,
        eva["y_anchor"],
        eva["y_prev"],
        rho=rho_best,
    ...
    summary = {
        "config": {
            ...
            "kappa_grid": [float(x) for x in kappa_grid.tolist()],
    ...
    (out_dir / "phase_state_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

    with (out_dir / "phase_state_grid_scan.csv").open("w", encoding="utf-8", newline="") as f:
    ...
    with (out_dir / "test_predictions.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["date_t1", "y_t1_actual", "y_t1_pred_raw", "y_t1_pred_state", "phase_state_z", "y_t", "y_t_minus_1"],
        )
        writer.writeheader()
            ...
            writer.writerow(
                {
                    ...
                    "phase_state_z": float(z_corr[i]),
                    "y_t": float(eva["y_anchor"][i]),
                    "y_t_minus_1": float(eva["y_prev"][i]),
                }
            )
    ...
    print("[done] step15 explicit-phase-state scan complete")
    print(
        "[done] chosen: "
        f"rho={rho_best:.4g}, kappa={kappa_best:.4g}, eta={eta_best:.4g}, "
