"""Step 12 experiment: learnable inertia coefficient rho for one-step dynamics.

Model form:
    y_{t+1} = y_t + rho * (y_t - y_{t-1}) + dt^2 * F_t
where F_t keeps the original nonlinear damping + forcing structure.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scan learnable inertia rho with train-fit / valid-select / test-eval.")
    ...
    parser.add_argument("--output-dir", default="data/us/experiments/step12_learnable_inertia_rho")
    ...
    parser.add_argument("--rho-max", type=float, default=1.2)
...
def _extract_rows(rows: list[dict[str, str]], feature_cols: list[str]) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    return dates[finite], y[finite], a[finite], u[finite]
...
def _fit_params_for_rho(
    ...
    *,
    ...
    # Move the learnable inertia term to lhs so identify_ols_y_next_panel can be reused.
    y_next_prime = y_next - (rho - 1.0) * (y_cur - y_prev)
...
def _predict_for_rho(
    ...
    v = (y_cur - y_prev) / dt
    dt2 = dt * dt
    forcing = intercept - (c / m) * v - (c_nl / m) * np.abs(v) * v - (k / m) * y_cur + (u[idx] @ beta_vec)
    y_hat = y_cur + rho * (y_cur - y_prev) + dt2 * forcing
    return y_true, y_hat, y_cur
...
def main() -> None:
    ...
    rows_fit = _split_rows(rows_all, args.fit_split)
    rows_sel = _split_rows(rows_all, args.selection_split)
    rows_eval = _split_rows(rows_all, args.eval_split)

    _, y_fit, a_fit, u_fit = _extract_rows(rows_fit, feature_cols=feature_cols)
    _, y_sel, _, u_sel = _extract_rows(rows_sel, feature_cols=feature_cols)
    _, y_eval, _, u_eval = _extract_rows(rows_eval, feature_cols=feature_cols)
    ...
    if args.rho_step <= 0:
        raise ValueError("--rho-step must be > 0")
    rho_grid = np.arange(args.rho_min, args.rho_max + 0.5 * args.rho_step, args.rho_step)

    candidates: list[dict[str, object]] = []
    for rho in rho_grid.tolist():
        try:
            est_obj = _fit_params_for_rho(
                y=y_fit,
                a=a_fit,
                u=u_fit,
            ...
            ys_true, ys_hat, ys_anchor = _predict_for_rho(y_sel, u_sel, feature_cols, est_obj, rho=float(rho), dt=args.dt)
            ye_true, ye_hat, ye_anchor = _predict_for_rho(y_eval, u_eval, feature_cols, est_obj, rho=float(rho), dt=args.dt)

            m_sel = _metrics(ys_true, ys_hat, ys_anchor)
            m_eval = _metrics(ye_true, ye_hat, ye_anchor)
            lag_sel = _lag_scan(ys_true, ys_hat, ys_anchor, max_lag=args.max_lag)
            lag_eval = _lag_scan(ye_true, ye_hat, ye_anchor, max_lag=args.max_lag)
            best_sel = _best_lag(lag_sel)
            best_eval = _best_lag(lag_eval)
            ...
            rec = {
                ...
                "selection": {
                    "metrics": m_sel,
                    "best_lag": int(best_sel["lag"]),
                    "best_lag_corr": float(best_sel["corr"]),
                    "best_lag_rmse": float(best_sel["rmse"]),
                },
                "eval": {
                    "metrics": m_eval,
                    "best_lag": int(best_eval["lag"]),
                    "best_lag_corr": float(best_eval["corr"]),
                    "best_lag_rmse": float(best_eval["rmse"]),
                    "turn": turn_eval,
                },
            }
            rec["gap_eval_vs_main"] = {
                ...
                "best_lag": int(best_eval["lag"] - int(baseline_phase["best_lag_by_corr"]["lag"])),
            ...
            rec = {"rho": float(rho), "status": f"failed: {e}"}
        candidates.append(rec)

    ok = [r for r in candidates if r.get("status") == "ok"]
    if not ok:
        raise RuntimeError("all rho candidates failed")

    # Selection priority: phase first (|best_lag|), then x_rmse, then x_corr.
    ...
    (out_dir / "learnable_rho_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

    with (out_dir / "learnable_rho_table.csv").open("w", encoding="utf-8", newline="") as f:
        ...
        for r in candidates:
            if r.get("status") != "ok":
                writer.writerow({"rho": r.get("rho"), "status": r.get("status")})
                continue
            writer.writerow(
                {
                    "rho": r["rho"],
    ...
    print("[done] step12 learnable-rho scan complete")
    print(f"[done] candidates={len(candidates)}, ok={len(ok)}")
    print(
        "[done] best_by_selection: "
        f"rho={best_selection['rho']:.4g}, sel_best_lag={best_selection['selection']['best_lag']}, "
        f"eval_best_lag={best_selection['eval']['best_lag']}, eval_x_r2={best_selection['eval']['metrics']['x_r2']:.6g}"
    )
    print(
        "[done] best_by_eval: "
        f"rho={best_eval['rho']:.4g}, eval_best_lag={best_eval['eval']['best_lag']}, "
