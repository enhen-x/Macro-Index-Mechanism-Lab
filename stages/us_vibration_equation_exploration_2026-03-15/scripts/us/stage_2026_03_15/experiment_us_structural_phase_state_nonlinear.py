"""Step 25 experiment: structural phase state with nonlinear drive + gated recursion.

Amplitude:
    delta_base_t = f_amp(z_t)

Phase (nonlinear + gated):
    drive_t = f_phase_nl(z_t, delta_base_t)
    gate_t = sigmoid(k_gate * gate_signal_t)
    rho_eff_t = rho_low * (1-gate_t) + rho_high * gate_t
    d_t = rho_eff_t * d_{t-1} + drive_t
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Structural nonlinear phase-state scan.")
    ...
    parser.add_argument("--output-dir", default="data/us/experiments/step25_structural_phase_state_nonlinear")
    ...
    parser.add_argument("--phase-ridge-grid", default="0.01,0.1")
    parser.add_argument("--rho-low-grid", default="0,0.3")
    parser.add_argument("--rho-high-grid", default="0.7,0.9")
    parser.add_argument("--gamma-grid", default="0.5,1.0,1.5")
    parser.add_argument("--gate-k-grid", default="2,4")
...
def _phase_basis(x_phase_raw: np.ndarray) -> np.ndarray:
    # nonlinear basis: [x, x^2, tanh(x)]
    return np.column_stack([x_phase_raw, x_phase_raw * x_phase_raw, np.tanh(x_phase_raw)])


def _fit_phase_drive(
    z: np.ndarray,
    ...
    *,
    ...
    d_prev[0] = 0.0
    y_tilde = d_target - float(rho_mid) * d_prev

    x_phase_raw = np.column_stack([z, delta_base])
    x_phase = _phase_basis(x_phase_raw)
...
def _predict_structural_nl(
    ...
    *,
    ...
    x_phase_raw = np.column_stack([z, delta_base])
    x_phase = _phase_basis(x_phase_raw)
    drive = _predict_ridge(x_phase, phase_model)

    std_db = float(np.std(delta_base)) + 1e-8
    std_acc = float(np.std(accel_t)) + 1e-8
    gate_signal = 0.5 * (np.abs(delta_base) / std_db + np.abs(accel_t) / std_acc) - 1.0
    gate = _sigmoid(float(gate_k) * gate_signal)

    d_prev = 0.0
    y_hat = np.zeros(len(y_anchor), dtype=float)
    d_series = np.zeros(len(y_anchor), dtype=float)
    for i in range(len(y_anchor)):
        rho_eff = float(rho_low) * (1.0 - float(gate[i])) + float(rho_high) * float(gate[i])
        d_t = rho_eff * d_prev + float(drive[i])
        d_t = float(np.clip(d_t, -float(d_clip), float(d_clip)))
        y_hat[i] = y_anchor[i] + delta_base[i] * (1.0 + float(gamma) * d_t)
        d_series[i] = d_t
        d_prev = d_t
    return y_hat, d_series, gate
...
def main() -> None:
    ...
    rho_low_grid = _parse_float_list(args.rho_low_grid)
    rho_high_grid = _parse_float_list(args.rho_high_grid)
    gamma_grid = _parse_float_list(args.gamma_grid)
    gate_k_grid = _parse_float_list(args.gate_k_grid)

    candidates: list[dict[str, object]] = []
    for pr in phase_ridge_grid:
        phase_fit = _fit_phase_drive(
            fit_ds["z"],
            delta_fit,
            fit_ds["y_anchor"],
            fit_ds["y_true"],
        ...
        for rlow in rho_low_grid:
            for rhigh in rho_high_grid:
                if rhigh < rlow:
                    continue
                for gk in gate_k_grid:
                    for gamma in gamma_grid:
                        y_sel_hat, d_sel, gate_sel = _predict_structural_nl(
                            sel_ds["z"],
                            sel_ds["y_anchor"],
                            delta_sel,
                            sel_ds["delta_t"],
                        ...
                        y_eval_hat, d_eval, gate_eval = _predict_structural_nl(
                            eval_ds["z"],
                            eval_ds["y_anchor"],
                            delta_eval,
                            eval_ds["delta_t"],
                        ...
                        rec = {
                            ...
                            "selection": {
                                ...
                                "turn": sel_turn,
                                "mean_abs_d_state": float(np.mean(np.abs(d_sel))),
                                "mean_gate": float(np.mean(gate_sel)),
                            },
                            "eval": {
                                "metrics": eval_m,
                                ...
                                "mean_gate": float(np.mean(gate_eval)),
                            },
                        }
    ...
    y_eval_hat, d_eval, gate_eval = _predict_structural_nl(
        ...
        rho_low=float(chosen["rho_low"]),
        rho_high=float(chosen["rho_high"]),
        gamma=float(chosen["gamma"]),
        gate_k=float(chosen["gate_k"]),
        d_clip=float(args.d_clip),
    )
    ...
    summary = {
        "config": {
            ...
            "rho_low_grid": rho_low_grid,
            "rho_high_grid": rho_high_grid,
            "gate_k_grid": gate_k_grid,
    ...
    (out_dir / "structural_phase_nl_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

    with (out_dir / "structural_phase_nl_scan.csv").open("w", encoding="utf-8", newline="") as f:
            ...
            writer.writerow(
                {
                    ...
                    "sel_mean_abs_d_state": r["selection"]["mean_abs_d_state"],
                    "sel_mean_gate": r["selection"]["mean_gate"],
                    ...
                    "eval_mean_gate": r["eval"]["mean_gate"],
    ...
    with (out_dir / "test_predictions_best_by_selection.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["date_t1", "y_t1_actual", "y_t1_pred_structural_nl", "y_t", "d_state", "gate", "phase_ridge", "rho_low", "rho_high", "gate_k", "gamma"],
        )
        writer.writeheader()
            ...
            writer.writerow(
                {
                    ...
                    "y_t1_pred_structural_nl": float(y_eval_hat[i]),
                    "y_t": float(eval_ds["y_anchor"][i]),
                    "d_state": float(d_eval[i]),
                    "gate": float(gate_eval[i]),
    ...
    print("[done] step25 structural-phase-state-nonlinear scan complete")
    print(
        "[done] best_by_selection: "
        f"phase_ridge={best_by_selection['phase_ridge']:.6g}, rho_low={best_by_selection['rho_low']:.6g}, "
        f"rho_high={best_by_selection['rho_high']:.6g}, gate_k={best_by_selection['gate_k']:.6g}, gamma={best_by_selection['gamma']:.6g}, "
    ...
    print(
        "[done] best_by_eval: "
        f"phase_ridge={best_by_eval['phase_ridge']:.6g}, rho_low={best_by_eval['rho_low']:.6g}, "
        f"rho_high={best_by_eval['rho_high']:.6g}, gate_k={best_by_eval['gate_k']:.6g}, gamma={best_by_eval['gamma']:.6g}, "
