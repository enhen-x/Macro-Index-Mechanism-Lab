"""Step 10 experiment: causal lead-compensation layer to reduce phase lag."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scan causal lead compensation on top of raw one-step forecasts.")
    ...
    parser.add_argument("--output-dir", default="data/us/experiments/step10_phase_lead_compensation")
    parser.add_argument("--selection-split", choices=["train", "valid", "test", "all"], default="train")
...
def _apply_comp(y_hat_raw: np.ndarray, y_anchor: np.ndarray, y_prev: np.ndarray, gamma: float, eta: float) -> np.ndarray:
    delta_pred = y_hat_raw - y_anchor
    delta_anchor = y_anchor - y_prev
    return y_hat_raw + gamma * delta_pred + eta * delta_anchor
...
def main() -> None:
    ...
    tr = _one_step_raw(_split_rows(rows_all, args.selection_split), feature_cols, baseline_est, dt=args.dt)
    te = _one_step_raw(_split_rows(rows_all, args.eval_split), feature_cols, baseline_est, dt=args.dt)

    gamma_grid = np.arange(0.0, 1.21, 0.1)
    eta_grid = np.arange(-0.5, 0.51, 0.1)

    rows_scan: list[dict[str, float]] = []
    for gamma in gamma_grid:
        for eta in eta_grid:
            y_hat = _apply_comp(tr["y_hat_raw"], tr["y_anchor"], tr["y_prev"], gamma=float(gamma), eta=float(eta))
            m = _metrics(tr["y_true"], y_hat, tr["y_anchor"])
            lag_scan = _lag_scan(tr["y_true"], y_hat, tr["y_anchor"], max_lag=args.max_lag)
            best = _best_lag(lag_scan)
            rows_scan.append(
                {
                    "gamma": float(gamma),
                    "eta": float(eta),
                    "train_x_r2": float(m["x_r2"]),
                    "train_x_rmse": float(m["x_rmse"]),
                    "train_x_corr": float(m["x_corr"]),
                    "train_best_lag": int(best["lag"]),
                    "train_best_lag_corr": float(best["corr"]),
                    "train_best_lag_rmse": float(best["rmse"]),
                }
            )

    def _rank_key(r: dict[str, float]) -> tuple[float, float, float]:
        return (abs(int(r["train_best_lag"])), -float(r["train_x_corr"]), float(r["train_x_rmse"]))

    best_row = sorted(rows_scan, key=_rank_key)[0]
    gamma_best = float(best_row["gamma"])
    eta_best = float(best_row["eta"])

    y_raw = te["y_hat_raw"]
    y_corr = _apply_comp(y_raw, te["y_anchor"], te["y_prev"], gamma=gamma_best, eta=eta_best)

    m_raw = _metrics(te["y_true"], y_raw, te["y_anchor"])
    m_corr = _metrics(te["y_true"], y_corr, te["y_anchor"])
    lag_raw = _lag_scan(te["y_true"], y_raw, te["y_anchor"], max_lag=args.max_lag)
    lag_corr = _lag_scan(te["y_true"], y_corr, te["y_anchor"], max_lag=args.max_lag)
    best_raw = _best_lag(lag_raw)
    best_corr = _best_lag(lag_corr)
    turn_raw = _turn_metrics(te["y_true"], y_raw, te["y_anchor"], eps=args.turn_eps, tol=args.turn_tol)
    turn_corr = _turn_metrics(te["y_true"], y_corr, te["y_anchor"], eps=args.turn_eps, tol=args.turn_tol)
    ...
    summary = {
        "config": {
            ...
            "eta_grid": [float(x) for x in eta_grid.tolist()],
        },
        "chosen": best_row,
    ...
    (out_dir / "lead_comp_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

    with (out_dir / "grid_scan.csv").open("w", encoding="utf-8", newline="") as f:
    ...
    with (out_dir / "test_predictions.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["date_t1", "y_t1_actual", "y_t1_pred_raw", "y_t1_pred_corr", "y_t", "y_t_minus_1"],
        )
        writer.writeheader()
        for i in range(len(te["y_true"])):
            writer.writerow(
                {
                    "date_t1": te["date_t1"][i].strftime("%Y-%m-%d"),
                    "y_t1_actual": float(te["y_true"][i]),
                    "y_t1_pred_raw": float(y_raw[i]),
                    "y_t1_pred_corr": float(y_corr[i]),
                    "y_t": float(te["y_anchor"][i]),
                    "y_t_minus_1": float(te["y_prev"][i]),
                }
            )

    print("[done] step10 lead-compensation scan complete")
    print(
        "[done] chosen: "
        f"gamma={gamma_best:.4g}, eta={eta_best:.4g}, train_best_lag={int(best_row['train_best_lag'])}, "
        f"train_x_r2={float(best_row['train_x_r2']):.6g}"
    )
    print(
        f"[done] raw->corr ({args.eval_split}): "
