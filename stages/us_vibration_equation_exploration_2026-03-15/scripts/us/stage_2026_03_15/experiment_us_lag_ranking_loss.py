"""Step 23 experiment: lag-ranking hard loss (enforce lag0 over lag-1).
...
Training objective:
    L = L_amp + lambda_phase*L_phase + lambda_turn*L_turn
        + lambda_rank * relu(margin + mse_lag0 - mse_lagm1)
        + L2(theta)
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scan lag-ranking hard loss.")
    ...
    parser.add_argument("--output-dir", default="data/us/experiments/step23_lag_ranking_loss")
    ...
    parser.add_argument("--phase-lambda-grid", default="0.2,0.5")
    parser.add_argument("--turn-lambda-grid", default="0,0.2")
    parser.add_argument("--ridge-grid", default="0.01,0.05")
    parser.add_argument("--rank-lambda-grid", default="0,0.5,1,2,5")
    parser.add_argument("--rank-margin-grid", default="0,1e-4,5e-4,1e-3")
...
def _prepare_dataset(rows: list[dict[str, str]], feature_cols: list[str], *, orthogonalize_country: bool) -> dict[str, np.ndarray]:
    ...
    x_nb = np.column_stack([y_t, delta_t, accel_t, u[idx]])
    return {
        "date_t1": dates[idx + 1],
        "y_true": y_tp1,
        "y_anchor": y_t,
...
def _standardize_fit(x_no_bias: np.ndarray) -> dict[str, np.ndarray]:
...
def _standardize_apply(x_no_bias: np.ndarray, mu: np.ndarray, sigma: np.ndarray) -> np.ndarray:
    xs = (x_no_bias - mu) / sigma
...
def _lag_rank_term(y_hat: np.ndarray, y_true: np.ndarray, margin: float) -> tuple[float, np.ndarray]:
    # lag0 mse
    n = len(y_hat)
    err0 = y_hat - y_true
    mse0 = float(np.mean(err0 * err0))
    g0 = (2.0 / n) * err0

    # lag-1 mse: compare y_hat[1:] with y_true[:-1]
    if n < 2:
        return 0.0, np.zeros(n, dtype=float)
    errm1 = y_hat[1:] - y_true[:-1]
    mse_m1 = float(np.mean(errm1 * errm1))
    gm1 = np.zeros(n, dtype=float)
    gm1[1:] = (2.0 / (n - 1)) * errm1

    hinge = float(margin + mse0 - mse_m1)
    if hinge <= 0:
        return 0.0, np.zeros(n, dtype=float)
    grad = g0 - gm1
    return hinge, grad
...
def _train_with_rank(
    ...
    *,
    ...
    for it in range(iters):
        ...
        rank_hinge, g_y_rank = _lag_rank_term(y_hat, y_true, margin=float(rank_margin))
        last_rank_hinge = rank_hinge
        grad_rank = x.T @ g_y_rank
        ...
        grad = (
            grad_amp
            + float(lambda_phase) * grad_phase
            + float(lambda_turn) * grad_turn
            + float(lambda_rank) * grad_rank
            + grad_reg
        )
...
def main() -> None:
    ...
    std = _standardize_fit(fit_ds["x_no_bias"])
    x_fit = std["x"]
    x_sel = _standardize_apply(sel_ds["x_no_bias"], std["mu"], std["sigma"])
    x_eval = _standardize_apply(eval_ds["x_no_bias"], std["mu"], std["sigma"])
    ...
    rank_lambdas = _parse_float_list(args.rank_lambda_grid)
    rank_margins = _parse_float_list(args.rank_margin_grid)

    candidates: list[dict[str, object]] = []
    for lp in phase_lambdas:
        for lt in turn_lambdas:
            for ridge in ridge_grid:
                for lrk in rank_lambdas:
                    for mrg in rank_margins:
                        fit_obj = _train_with_rank(
                            x_fit,
                            fit_ds["y_anchor"],
                            fit_ds["y_true"],
                            fit_ds["delta_true"],
                            ...
                            lambda_rank=float(lrk),
                            rank_margin=float(mrg),
                        ...
                        rec = {
                            ...
                            "fit": {
                                ...
                                "rank_hinge": float(fit_obj["fit_rank_hinge"]),
                            },
                            "selection": {
                                "metrics": sel_m,
    ...
    fit_obj = _train_with_rank(
        ...
        lambda_rank=float(chosen["lambda_rank"]),
        rank_margin=float(chosen["rank_margin"]),
    ...
    summary = {
        "config": {
            ...
            "rank_lambda_grid": rank_lambdas,
            "rank_margin_grid": rank_margins,
            "turn_k": float(args.turn_k),
            "lr": float(args.lr),
            "iters": int(args.iters),
    ...
    (out_dir / "lag_ranking_loss_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

    with (out_dir / "lag_ranking_loss_scan.csv").open("w", encoding="utf-8", newline="") as f:
            ...
            writer.writerow(
                {
                    ...
                    "fit_rank_hinge": r["fit"]["rank_hinge"],
    ...
    with (out_dir / "test_predictions_best_by_selection.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["date_t1", "y_t1_actual", "y_t1_pred_rankloss", "y_t", "lambda_phase", "lambda_turn", "lambda_rank", "rank_margin", "ridge"],
        )
        writer.writeheader()
            ...
            writer.writerow(
                {
                    ...
                    "y_t1_pred_rankloss": float(y_eval_hat[i]),
    ...
    print("[done] step23 lag-ranking-loss scan complete")
    print(
        "[done] best_by_selection: "
        f"lambda_phase={best_by_selection['lambda_phase']:.6g}, lambda_turn={best_by_selection['lambda_turn']:.6g}, "
        f"lambda_rank={best_by_selection['lambda_rank']:.6g}, rank_margin={best_by_selection['rank_margin']:.6g}, "
    ...
    print(
        ...
        f"lambda_rank={best_by_eval['lambda_rank']:.6g}, rank_margin={best_by_eval['rank_margin']:.6g}, "
